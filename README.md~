...
###  复杂度估计：
	由于用的是深度搜索，深度是单词对应位置上不同字母的个数，故可把深度看作常数;每一层的宽度是该单词的相近单词个数，故宽度也可看作常数;故总的复杂度应该是O(c)，但需要指出的是：由于我们把数据处理放到了另一个项目中完成，所以复杂度才变为常数级;另外，搜索的复杂度其实跟单词个数的规模无关，而是跟所选单词之间的关系有关。
###  bug：
